...
const CHANGE_BG: Record<ChangeType, string> = {
  '신규': '#FDF0E1', '토글 분리': '#EAF1FF', '명칭 변경': '#F3EAFE', '표기 정리': '#EFF7EF', '유지': '#F3F3F5',
};
const CHANGE_FG: Record<ChangeType, string> = {
  '신규': '#B45309', '토글 분리': '#2563EB', '명칭 변경': '#7C3AED', '표기 정리': '#1E8F1B', '유지': '#71717A',
};
...
export function Overview() {
  ...
  const scopeRows: { in: boolean; t: string; d: string }[] = [
    { in: true, t: '포함', d: '기본설정 페이지 맨 위 설정 영역. 카테고리 탭 8개, 설정 35개, 상태 토글, 초성 인덱스' },
    { in: false, t: '제외', d: '같은 페이지 아래 5개 영역(쇼핑몰 정보, 플로팅 액션 버튼 설정, 쇼핑몰 하단 안내, 회사소개, SNS 링크). 기존 동일' },
  ];
  const screenAreas: [string, string, string][] = [
    ['카테고리 탭', '설정을 성격별로 나눠 보는 알약형 탭 8개', '전체, 운영 형태, 화면·노출, 상품·장바구니, 구매후기, 주문·결제, 보안, 정산·재고'],
  ...
  const flowSteps = [
    '디자인관리 기본설정으로 들어가면 전체 탭에 설정 35개가 가나다순으로 펼쳐진다.',
          ...
          <Box flex="1" minW="0">
            ...
            <Sec id="compare" n="06" t="As-Is / To-Be 비교">
              <Text fontSize="14px" color="#71717A" lineHeight="1.7" pb="16px">
                개편 전에는 설정 35개가 한 페이지에 평면으로 나열됐다. 기능을 쓸지 말지가 옵션 목록에 섞여 있었고, 토글이 있는 행과 없는 행이 번갈아 나와 목록을 훑는 기준이 없었다. 개편에서는 35개가 카테고리 탭 8개로 나뉘고, 사용 여부는 전부 상태 토글로 빠진다.
              </Text>
              ...
              <DTable
                ...
                rows={[
                  ['목록 구조', '한 페이지에 35개를 평면 나열', '카테고리 탭 8개, 가나다순, 초성 인덱스 뱃지'],
